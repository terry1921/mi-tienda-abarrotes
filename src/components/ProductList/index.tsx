...
const ProductList: React.FC<ProductListProps> = ({category}) => {
    ...
    const {filteredProducts} = useCategory();
    ...
    const show: boolean = category.id === filteredProducts[0].category;

    if (show) {
        return (
            <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-2 gap-2 px-2">
                {
                    filteredProducts.map((product) => (
                        <div onClick={() => addToCart(product, 1)} key={product.id} className="bg-card-light border rounded-md p-4 shadow-lg">
                            <div className="flex justify-center items-center">
                                <svg width="50" height="50" fill="currentColor" aria-hidden="true"
